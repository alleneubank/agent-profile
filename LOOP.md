...
- Branch (work loop home) `main`; dogfood branch `feat/pi-package-support` pushed as PR #1 (open, unmerged — see boundary). Tree clean.
- **Dogfood E2E executed (unit 4 evidence, ahead of plan):** throwaway exe.dev VM (`pi-pkg-dogfood.exe.xyz`, exeuntu, created + destroyed same day). On a clean host: node 22 + pi 0.84.1 installed, `pi install https://github.com/alleneubank/agent-profile@feat/pi-package-support` succeeded, RPC probe showed **41 package skills** in `get_commands` (paths under the package git dir) and a **`custom_message instruction-fingerprint` entry** in `get_entries` — the session_start hook ran end-to-end in the real harness (content `unknown`, the script's correct fail-open degradation on hosts without a claude-bootstrap checkout). No `extension_error` events.
- **Critical dogfood finding:** remote `main` predates unit 1 (`dbb5992`), so `pi install https://github.com/alleneubank/agent-profile` (no ref) clones the old layout and loads **zero** resources. The manifest exists only on the unmerged feature branch. Until PR #1 merges (or a tag is cut), the installation path that every other user will use is a no-op — merge is the real enabler for the fleet-as-pi-packages approach; document "install from main after merge" in README once merged.
- Unit 2 (recall) and unit 3 (dotfiles settings retirement) remain pending; unit 5+6 (fleet manifests, review gate) uncharted.
