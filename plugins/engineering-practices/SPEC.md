...
- **Code law** is reusable craft judgment whose violations are caught by a
  harness or review gate; unrecoverable boundary law remains in `AGENTS.md`.
- A **scenario oracle** gives a fresh-context reviewer fixed engineering choices
  and grades whether the skills produce the intended judgment without access to
...
- **REQ-DOCTRINE-006 — Verification:** fixed scenarios are observed red against
  the prior skills and pass a fresh-context review after the revision; the
...
- The two existing skills are revised in place; no new runtime skill is created.
  (2026-08-29, ratified)
- Behavioral scenario review complements, rather than enters, the deterministic
  `npm` and validation gates. (2026-08-29, ratified)
...
- [x] A fresh-context scenario reviewer reports no material-or-higher finding.
...
- REQ-DOCTRINE-001 through REQ-DOCTRINE-005 — fixed behavioral cases in
  ...
  findings.
- REQ-DOCTRINE-006 — `npm run check`, `./scripts/validate.sh`, and the fixed
  scenario oracle.
